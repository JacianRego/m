...
for i in range(len(white_notes)):
    white_sounds.append(mixer.Sound(f'assets\\music_notes\\{white_notes[i]}.wav'))

for i in range(len(black_notes)):
    black_sounds.append(mixer.Sound(f'assets\\music_notes\\{black_notes[i]}.wav'))

pygame.display.set_caption("Piano for Maryam")
...
def draw_title_bar():
    instruction_text = medium_font.render('Up/Down Arrows change the left hand', True, 'black')
    screen.blit(instruction_text, (WIDTH - 500, 10))
    instruction_text2 = medium_font.render('Left/Right Arrows change the right hand', True, 'black')
    screen.blit(instruction_text2, (WIDTH - 500, 50))
    title_text = font.render('wayTooMuchTime', True, 'black')
